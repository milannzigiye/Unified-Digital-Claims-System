...
$claimant_name = (string) ($user_data['full_name'] ?? 'Legal Officer');
...
<head>
    <?php
    render_head(
        'Legal Messaging | UNIFIED DIGITAL CLAIMS SYSTEM',
        '..',
...
<main class="main-content px-4 pb-8 pt-4 sm:px-6 lg:px-8">
    <section class="chat-hero p-3 sm:p-4">
        <h1 class="mt-1 font-display text-2xl font-bold text-bk-text sm:text-3xl">Legal Messaging</h1>
        <p class="mt-1 max-w-2xl text-sm text-bk-muted">Coordinate with claimants and finance teams for legal reviews and claim handoffs.</p>
    </section>
            ...
            <div class="contact-list" id="contactList">
                ...
                <?php else: ?>
                    <?php foreach ($chat_users as $user): ?>
                        ...
                        $roleClass = $role === 'finance' ? 'role-finance' : ($role === 'claimant' ? 'role-claimant' : 'role-default');
