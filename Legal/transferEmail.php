...
$actionPayload = udcs_action_token_consume('legal_transfer_email', $actionToken);
if (!is_array($actionPayload)) {
    $_SESSION['alertMessage'] = 'Transfer token is missing or expired.';
...
$claim = udcs_fetch_claim_for_email($conn, $primaryEmail, ['transferred to finance', 'approved by legal'], $claimId);
if (!$claim) {
    $_SESSION['alertMessage'] = 'No claim was found for transfer notification.';
    $_SESSION['alertType'] = 'danger';
    header('Location: claims.php');
    exit();
}
...
$decisionNote = udcs_extract_decision_note((string) ($claim['comment'] ?? ''), 'legal_to_finance');
$claimCurrency = bk_currency_code((string) ($claim['claim_currency_code'] ?? 'RWF'));

$payload = udcs_render_claim_email_payload('legal_to_finance', [
    'id' => $claimId,
...
try {
    ...
    $_SESSION['alertMessage'] = 'Transfer notification email sent successfully to ' . count($recipients) . ' recipient(s).';
    $_SESSION['alertType'] = 'success';
    header('Location: claims.php');
    exit();
} catch (Exception $e) {
    error_log('transferEmail.php send failed: ' . $mail->ErrorInfo);
    $_SESSION['alertMessage'] = 'Transfer notification email could not be sent. Please try again.';
    $_SESSION['alertType'] = 'danger';
    header('Location: claims.php');
    exit();
}
