...
$identity = udcs_notifications_identity_from_session($conn, 'claimant');
if (!$identity) {
    respond_json(false, 'Not authenticated.');
}

$openedTo = udcs_notifications_mark_opened(
    $conn,
    (int) $identity['user_id'],
    (string) $identity['email'],
    (int) ($_POST['latest_id'] ?? 0)
);

respond_json(true, 'Notifications panel state updated.', ['opened_to' => $openedTo]);
