...
function admin_report_status_key(?string $status): string
{
...
if (!$stmt) {
    udcs_exit_public_error('Admin claims export failed during statement preparation.', 'The claims review report could not be generated right now.', $conn);
}

if (!udcs_db_stmt_bind($stmt, $types, $params) || !mysqli_stmt_execute($stmt)) {
    udcs_exit_public_error('Admin claims export failed during statement execution.', 'The claims review report could not be generated right now.', $conn);
}

$result = mysqli_stmt_get_result($stmt);
if (!$result) {
    udcs_exit_public_error('Admin claims export failed while reading the result set.', 'The claims review report could not be generated right now.', $conn);
}
...
$pdf = udcs_pdf_create_document([
    'title' => 'Claims Review Export',
    'portal' => 'Admin Portal',
    'model' => 'Claims Oversight Model',
...
$columns = [
    ...
    ['key' => 'deceased', 'label' => 'Deceased', 'weight' => 1.25, 'align' => 'L'],
    ['key' => 'status', 'label' => 'Status', 'weight' => 1.0, 'align' => 'L'],
    ['key' => 'assets', 'label' => 'BK Assets', 'weight' => 1.45, 'align' => 'L'],
    ['key' => 'value', 'label' => 'Value', 'weight' => 1.0, 'align' => 'R'],
...
$pdf->Output('claims_review_admin_' . date('Ymd_His') . '.pdf', 'I');
exit();
