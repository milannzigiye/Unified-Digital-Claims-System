...
<head>
    <?php
    render_head(
        'Admin Messaging | UNIFIED DIGITAL CLAIMS SYSTEM',
        '..',
...
<main class="main-content px-4 pb-8 pt-4 sm:px-6 lg:px-8">
    <section class="chat-hero p-3 sm:p-4">
        <h1 class="mt-1 font-display text-2xl font-bold text-bk-text sm:text-3xl">Admin Messaging</h1>
        <p class="mt-1 max-w-2xl text-sm text-bk-muted">Coordinate with claimants, legal, and finance teams in one conversation area.</p>
    </section>
            ...
            <div class="contact-list" id="contactList">
                ...
                <?php else: ?>
                    <?php foreach ($chat_users as $user): ?>
                        ...
                        $roleClass = $role === 'legal'
                            ? 'role-legal'
                            : ($role === 'finance' ? 'role-finance' : ($role === 'claimant' ? 'role-claimant' : 'role-default'));
