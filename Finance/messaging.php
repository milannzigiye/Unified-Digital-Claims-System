...
$claimant_name = (string) ($user_data['full_name'] ?? 'Finance Officer');
$rawPhoto = (string) ($user_data['photo'] ?? '');
...
<head>
    <?php
    render_head(
        'Finance Messaging | UNIFIED DIGITAL CLAIMS SYSTEM',
        '..',
...
<main class="main-content px-4 pb-8 pt-4 sm:px-6 lg:px-8">
    <section class="chat-hero p-3 sm:p-4">
        <h1 class="mt-1 font-display text-2xl font-bold text-bk-text sm:text-3xl">Finance Messaging</h1>
        <p class="mt-1 max-w-2xl text-sm text-bk-muted">Collaborate with claimants and legal staff to resolve payment and settlement workflows.</p>
    </section>

    <section class="chat-layout mt-4">
        <aside class="contact-panel">
            <header class="border-b border-bk-border px-3 py-2.5">
                <h2 class="font-display text-lg font-semibold text-bk-text">Contacts</h2>
            ...
            <div class="contact-list" id="contactList">
                <?php if (count($chat_users) === 0): ?>
                    <?php render_alert('No contacts are available right now.', ['type' => 'warning']); ?>
                <?php else: ?>
                    <?php foreach ($chat_users as $user): ?>
                        ...
                        $roleClass = $role === 'legal' ? 'role-legal' : ($role === 'claimant' ? 'role-claimant' : 'role-default');
        ...
        <section class="chat-panel">
            ...
            <div class="messages-container" id="messages-container">
                <div class="no-chat-selected" id="no-chat-selected">
                    ...
                    <p class="mt-1 text-xs">Choose a role filter, then open a specific person. Messages are private direct conversations, not broadcasts.</p>
                </div>
            </div>
...
(() => {
    ...
    function selectContact(item) {
        ...
        const userName = item.dataset.userName || 'User';
